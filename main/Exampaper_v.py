...
def exampaper_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'exampaper' != 'forum' :
            if req_dict.get("userid") and 'exampaper' != 'chat' and 'exampaper' != 'examrecord':
                del req_dict["userid"]
        else:
...
def exampaper_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM exampaper {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
