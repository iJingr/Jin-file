...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider${spiderSchemaName} && scrapy crawl "+tableName+"Spider -a databaseName=django5hdklc84"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=django5hdklc84"
        os.system(cmd)
...
def schemaName_mysqldump(request):
    if request.method in ["POST", "GET"]:
        ...
        cmd = "D:\\学习\\MySql\\mysql-5.7.29-winx64\\bin\\mysqldump -h127.0.0.1 -uroot -P3306 -p123456 django5hdklc84 > " + filePath
        os.system(cmd)
