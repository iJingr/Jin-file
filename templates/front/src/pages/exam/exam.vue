<template>
  <div>
		<div class="top-content">
			...
			<div class="top-content-center" v-cloak>距离心理测试结束还有 <el-tag type="danger">{{this.hour}}</el-tag> 时 <el-tag
					type="danger">{{this.minute}}</el-tag> 分 <el-tag type="danger">{{this.second}}</el-tag> 秒</div>
			<div class="top-content-right">
				<el-button type="danger" @click="doneExam(0)">结束心理测试</el-button>
			</div>
		</div>
		<el-card v-if="end" style="width: 90%;margin: 10px auto;">
			<div style="width: 100%;display: flex;flex-direction: column;align-items: center;">
				<div>
					<div class="question-title">心理测试成绩：<el-tag type="danger">{{scoreChange()}}</el-tag> 分</div>
				</div>
				<div class="question-btn">
					<el-button v-if="!isSubmit" type="primary" @click="isSubmit=true">查看解析</el-button>
					<el-button type="success" @click="doneExam(1)">结束心理测试</el-button>
				</div>
			</div>
		</el-card>
...
<script>
    ...
    //数据集合
		...
		methods: {
			...
			doneExam(type) {
				if (type == 0) {
					this.$confirm('确定离开心理测试?未答题目按0分计', '提示', {
						confirmButtonText: '确定',
						cancelButtonText: '取消',
						type: 'warning'
					}).then(() => {
