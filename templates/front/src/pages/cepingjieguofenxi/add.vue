...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="测评日期" prop="cepingriqi">
              <el-date-picker
				  :disabled=" false  ||ro.cepingriqi"
                  format="yyyy 年 MM 月 dd 日"
                  value-format="yyyy-MM-dd"
                  v-model="ruleForm.cepingriqi" 
                  type="date"
                  placeholder="测评日期">
              </el-date-picker> 
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="测评结果" prop="cepingjieguo">
            <el-input
              ...
              :rows="8"
              placeholder="测评结果"
              v-model="ruleForm.cepingjieguo">
            </el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="测评分析" prop="cepingfenxi">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="测评分析"
              v-model="ruleForm.cepingfenxi">
            </el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.cepingriqi = this.getCurDate()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='cepingfenxi'){
              this.ruleForm.cepingfenxi = obj[o];
              this.ro.cepingfenxi = true;
              continue;
            }
            if(o=='cepingriqi'){
              this.ruleForm.cepingriqi = obj[o];
              this.ro.cepingriqi = true;
              continue;
            }
          }
        }else if(type=='edit'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`cepingjieguofenxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('cepingjieguofenxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`cepingjieguofenxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
