...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="测评结果" prop="cepingjieguo">
            <el-input v-model="ruleForm.cepingjieguo" 
                placeholder="测评结果" clearable :disabled=" false  ||ro.cepingjieguo"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="辅导方案" prop="fudaofangan">
            <el-input v-model="ruleForm.fudaofangan" 
                placeholder="辅导方案" clearable :disabled=" false  ||ro.fudaofangan"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}'  label="效果评估" prop="xiaoguopinggu">
            <el-select v-model="ruleForm.xiaoguopinggu" placeholder="请选择效果评估" :disabled=" false  ||ro.xiaoguopinggu" >
              <el-option
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`fudaoxiaoguopinggu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('fudaoxiaoguopinggu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`fudaoxiaoguopinggu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
