	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="测评结果" prop="cepingjieguo">
					<el-input v-model="ruleForm.cepingjieguo" placeholder="测评结果" clearable  :readonly="ro.cepingjieguo"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="测评结果" prop="cepingjieguo">
					<el-input v-model="ruleForm.cepingjieguo" placeholder="测评结果" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="辅导方案" prop="fudaofangan">
					<el-input v-model="ruleForm.fudaofangan" placeholder="辅导方案" clearable  :readonly="ro.fudaofangan"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="辅导方案" prop="fudaofangan">
					<el-input v-model="ruleForm.fudaofangan" placeholder="辅导方案" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="效果评估" prop="xiaoguopinggu">
					<el-select :disabled="ro.xiaoguopinggu" v-model="ruleForm.xiaoguopinggu" placeholder="请选择效果评估" >
						<el-option
							v-for="(item,index) in xiaoguopingguOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="效果评估" prop="xiaoguopinggu">
					<el-input v-model="ruleForm.xiaoguopinggu"
						placeholder="效果评估" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="评估内容" prop="pingguneirong">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.pingguneirong" label="评估内容" prop="pingguneirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.pingguneirong}}</span>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fudaofangan'){
							this.ruleForm.fudaofangan = obj[o];
							this.ro.fudaofangan = true;
							continue;
						}
						if(o=='xiaoguopinggu'){
							this.ruleForm.xiaoguopinggu = obj[o];
							this.ro.xiaoguopinggu = true;
							continue;
						}
						if(o=='pingguneirong'){
            ...
            this.xiaoguopingguOptions = "好,一般,差".split(',')
    ...
    info(id) {
      this.$http({
        url: `fudaoxiaoguopinggu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `fudaoxiaoguopinggu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fudaoxiaoguopingguCrossAddOrUpdateFlag = false;
											this.parent.search();
